...
class configuration():
    ...
    def run_single_count(self, strategy, count):
        run_name = f"{self.model}-{strategy}-{self.max_trial}-{self.dataset}-{self.language}-{self.temperature}-{self.pass_at_k}-{count}"
...
if __name__ == "__main__":
    human_config = configuration("CC",["MapCoder_without_plan"])
    human_config.run()
